...
passes_sorted_even = passes_unique.sort_values(axis = 0, by = 'matchBalance')

passes_ratio = passes_sorted_even.head(int(len(passes_sorted_even)*ratio))

#%%
matches = pd.Series(passes_ratio['matchId'].unique())

#%%
#matches = pd.Series(passes['matchId'].unique())
#%% remove var events to clear memory
del events
...
#%%  prepare dictionary to store results in 
#reduce to matches and mirror y axis for better k means 
passes = passes.loc[passes['matchId'].isin(matches)]
...
for train_ix, test_ix in outer_cv.split(matches):
    ...
    #train best models found on outer train set and test on outer test
